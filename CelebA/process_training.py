...
def main(args):
    ...
    if args.dataset == 'CelebA':
        metadata_path = "./celebA/data/mmetadata.csv"
        #metadata_path = "./results/CelebA/CelebA_sample_exp/metadata_aug.csv"
        #metadata_path = 'results/CelebA/CelebA_sample_exp/train_downstream_ERM_upweight_0_epochs_50_lr_1e-05_weight_decay_0.1/final_epoch1/metadata_aug.csv'
    ...
    #    if train_df.loc[i, f"y_true_None_epoch_{final_epoch}_val"] == 0:
    #        train_df.loc[i, 'wrong_1_times'] = 0
    ...
    if dataset == "CUB":
        ...
        assert 0 == 0
        print(np.sum(merged_csv[merged_csv["split"] == 0]["Male"] != merged_csv[merged_csv["split"] == 0][f"y_true_None_epoch_{final_epoch}_val"]))
        assert 0 == np.sum(merged_csv[merged_csv["split"] == 0]["Male"] != merged_csv[merged_csv["split"] == 0][f"y_true_None_epoch_{final_epoch}_val"])
        #merged_csv["spurious"] = (merged_csv["Male"] == merged_csv["Eyeglasses"]) 
        merged_csv["spurious"] = (merged_csv["Male"] * merged_csv["Eyeglasses"]) 
    elif dataset == "jigsaw":
    ...
    merged_csv["our_nonspurious"] = (merged_csv["spurious"] == 0) & merged_csv["wrong_1_times"] & (merged_csv["Male"] == 1)
    ...
    if dataset == "MultiNLI":
        ...
        train_probs_df["probs_1"] = probs[:,1]
        if dataset == 'CelebA':
            train_probs_df["confidence"] = train_probs_df["Male"] * train_probs_df["probs_1"] + (1 - train_probs_df["Male"]) * train_probs_df["probs_0"]
        elif dataset == 'CUB':
