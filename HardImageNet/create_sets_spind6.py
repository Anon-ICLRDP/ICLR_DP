...
for i in all_indices:
    for j in all_indices[i][:int(len(all_indices[i])//10)]:
        all_to_skip.add(j)
...
for idx, c in enumerate(path_list_train):
    ...
    for im in curr_images:
        ...
        try:
            ...
            continue
        if (c == 'n03218198' and curr_rank >= 100) or (c =='n04228054' and curr_rank <= 900):
            #if (c =='n04228054' and curr_rank >= 500):
            continue
        other = t1(image)
        if other.size()[0] == 1:
...
torch.save(train_loader, 'train_loader_10.pth')
torch.save(val_loader, 'val_loader_10.pth')
