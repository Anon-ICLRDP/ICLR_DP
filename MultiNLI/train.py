...
def run_epoch(epoch, model, optimizer, loader, loss_computer, logger, csv_logger, args,
    ...
    with torch.set_grad_enabled(is_training):
        for batch_idx, batch in enumerate(prog_bar_loader):
            #print(len(prog_bar_loader.dataset))
            #torch.save(prog_bar_loader, 'trainloader.pt')
        ...
        if epoch in {4} and is_training:
            ...
            for batch_idx, batch in enumerate(prog_bar_loader):
                ...
                probs = F.softmax(outputs).detach()
                for j in range(len(probs)):
                    label = y[j].item()
                    curr_group = g[j].item()
                    curr_index = data_idx[j].item()
                    vector = torch.tensor([0, 0, 0]).cuda()
                    vector[label] = 1
                    el2n = torch.norm(vector - probs[j])
                    all_.append([curr_index, el2n, label, curr_group])
            with open(str(epoch) + 'el2n.pkl', 'wb') as f:
                pickle.dump(all_, f)

            torch.save(model, str(epoch) + '_model.pt')
            model.train()
